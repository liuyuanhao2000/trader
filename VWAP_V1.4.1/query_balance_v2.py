...
# 本地时间 + _time_offset_ms = Binance 服务器时间，缓解 WSL2 / 系统时钟漂移导致的 -1021
_time_offset_ms = 0
...
def sync_time_offset():
    ...
    print(f"[time-sync] offset={_time_offset_ms}ms (local + offset = server)")


def signed_request(method, path, params):
    params["timestamp"] = (time.time_ns() // 1_000_000) + _time_offset_ms
...
def get_balance():
    ...
    if "balances" not in account:
        # 一次重试：偏移可能因长时间运行而漂移，重算后再试一次
        sync_time_offset()
        account = signed_request("GET", "/api/v3/account", {})
    if "balances" not in account:
        raise RuntimeError(f"Binance API error: {account}")
    balances = []
